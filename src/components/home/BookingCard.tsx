...
export default function BookingCard() {
  ...
  const checkEvent = async () => {
    try { const r = await fetch(`/api/events?date=${date}&time=${time}`); const d = await r.json(); return d.event || null; } catch { return null; }
  };
      ...
      <div className="bg-white/[0.07] backdrop-blur-2xl border border-white/15 rounded-3xl p-6 sm:p-7 shadow-2xl shadow-black/20">
        ...
        <div className="space-y-2.5">
          <div className="grid grid-cols-2 gap-2.5">
            <div className={rowCls}><User className="w-4 h-4 text-white/30 shrink-0" /><label htmlFor="hero-name" className="sr-only">Name</label>
              <input id="hero-name" type="text" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} /></div>
            <div className={rowCls}><Phone className="w-4 h-4 text-white/30 shrink-0" /><label htmlFor="hero-phone" className="sr-only">Phone</label>
              <input id="hero-phone" type="tel" placeholder="Phone" value={phone} onChange={(e) => setPhone(e.target.value)} className={inputCls} /></div>
          </div>
          ...
          {stops.map((_, i) => (
            <div key={i} className="relative">
              ...
              <button type="button" onClick={() => { setStops((s) => s.filter((_, j) => j !== i)); setResult(null); }}
                className="absolute top-1/2 -translate-y-1/2 right-2 p-1 text-white/30 hover:text-crimson cursor-pointer"><X className="w-3.5 h-3.5" /></button>
            </div>))}
          {stops.length < 5 && (
          ...
          <HeroAddressInput id="hero-dropoff" placeholder="Drop-off Location" icon={<Navigation className="w-4 h-4 text-crimson" />} iconBg="bg-crimson/20"
            ready={mapsLoaded} onSelect={(p) => { setDropoff(p); setResult(null); }} onDetailsChange={setDropoffDetails} />
          <div className="grid grid-cols-2 gap-2.5">
            {(Object.entries(VEHICLES) as [VehicleType, typeof VEHICLES.car][]).map(([key, v]) => (
              <button key={key} type="button" onClick={() => { setVehicle(key); setResult(null); }}
                className={`flex items-center gap-2 rounded-xl px-4 py-3 border text-sm font-medium transition-all cursor-pointer ${vehicle === key ? "bg-gold/20 border-gold/50 text-gold" : "bg-white/10 border-white/10 text-white/60 hover:border-white/25"}`}>
          ...
          <div className="flex items-center gap-2.5">
            <div className="grid grid-cols-2 gap-2.5 flex-1">
              <div className={rowCls}><Calendar className="w-4 h-4 text-white/30 shrink-0" /><label htmlFor="hero-date" className="sr-only">Date</label>
                <input id="hero-date" type="date" value={date} min={(() => { const n = new Date(); return `${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,"0")}-${String(n.getDate()).padStart(2,"0")}`; })()} onChange={(e) => setDate(e.target.value)} className={`${inputCls} dark-picker ${!date ? "text-white/35" : ""}`} /></div>
              <div className={rowCls}><Clock className="w-4 h-4 text-white/30 shrink-0" /><label htmlFor="hero-time" className="sr-only">Time</label>
                <input id="hero-time" type="time" value={time} onChange={(e) => setTime(e.target.value)} className={`${inputCls} dark-picker ${!time ? "text-white/35" : ""}`} /></div>
            </div>
            <button type="button" onClick={() => { const now = new Date(); const y = now.getFullYear(), m = String(now.getMonth()+1).padStart(2,"0"), d = String(now.getDate()).padStart(2,"0"); setDate(`${y}-${m}-${d}`); setTime(now.toTimeString().slice(0, 5)); setResult(null); }}
              className="shrink-0 bg-gold/20 hover:bg-gold/30 border border-gold/40 text-gold text-xs font-bold rounded-xl px-3 py-3 transition-all cursor-pointer">Now</button>
          </div>
        </div>
        {error && <p className="text-red-400 text-xs mt-2">{error}</p>}
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={getQuote} disabled={loading}
          ...
          {loading ? "Calculating..." : "Get Instant Quote"}{!loading && <ArrowRight className="w-4 h-4" />}
        </motion.button>
        <AnimatePresence>
          {result && pickup && (
