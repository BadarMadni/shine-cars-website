...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, phone, pickup, dropoff, stops, date, time, distance, fare, vehicle, source, isPriority, priorityCharge, pickupDetails, dropoffDetails, buildingInfo } = body;
    ...
    if (isPriority) params.append("metadata[isPriority]", "true");
    if (priorityCharge) params.append("metadata[priorityCharge]", String(priorityCharge));
